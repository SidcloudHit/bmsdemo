...
<html>
    <head>        
        ...
        <title>Agriculture, Tripura || Beneficiary Drilled Down Report</title>
                            ...
                            <table cellpadding="0" cellspacing="0" border="0" class="table table-sm table-bordered table-hover small"  id="beneficiaryTab" style="display:none;">
                                <thead>
                                    <tr>
                                        ...
                                        <th>#</th>
                                        <th>RC No/ HOF Name(RC Member ID)</th>
                                        <th>Beneficiary Name(RC Member ID)</th>
                                        <th>Gender</th>
                                        <th>Caste</th>
...
<script type="text/javascript">
    ...
    function get_json_ben(id, parent_id) { //, callback
        var html_code = '';
        var i = 1;
        //console.log(id);
        //console.log(parent_id);
        ...
        $.each(JSON.parse(benListJson), function (key, value) {
            if (value.parent_code === parent_id) {
                html_code += "<tr>" +
                        "<td>" + i + "</td>" +
                        "<td>" + value.rationcard_memberid.substr(0,12) +"/"+value.hof_name +"("+value.hof_rc_member_id+")" + "</td>" +
    ...
    $(document).ready(function ()
    {
        $('#btnSearch').on('click', function (e) {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/DrilledDownReport?ddr1=" + reqType,
                type: "POST",
        ...
        $('#distTab').on("click", "td.control", function ()
        {
            ...
            if (i === -1)
            {
                ...
                var mtable = $('#subDivTab').html();
                //console.log(mtable);
                //console.log(nTr);


                //var nDetailsRow = oTable.fnOpen(nTr, fnFormatDetails(itable, iPos), 'details');
                var nDetailsRow = oTable.fnOpen(nTr, fnFormatDetails(mtable, iPos), 'details');
                ...
                $('#inner' + iPos).on("click", "td.control", function ()
                {
                    ...
                    if (j === -1)
                    {
                        ...
                        var ktable = $('#blockTab').html();

                        var nDetailsRow1 = oInnerTable.fnOpen(nTr1, fnFormatDetails1(ktable, iPos, jPos, 2), 'details');
                        ...
                        $('#doubleinner2_' + iPos + '_' + jPos).on("click", "td.control", function ()
                        {
                            ...
                            var l = $.inArray(nTr2, anOpen2);
                            if (l === -1)
                            {
                                var mPos = oInnerTable1.fnGetPosition(nTr2);
                                $('i', this).attr('class', "fa fa-minus");
                                get_json_data('gpTab', $('input', this).val());
                                var ktable = $('#gpTab').html();

                                var nDetailsRow1 = oInnerTable1.fnOpen(nTr2, fnFormatDetails1(ktable, jPos, mPos, 3), 'details');
                                $('div.innerDetails', nDetailsRow1).slideDown();
                                anOpen2.push(nTr2);
                                var oInnerTable2 = $('#doubleinner3_' + jPos + '_' + mPos).dataTable({
                                    "bRetrieve": true,
                                    "bJQueryUI": true,
                                    "bSort": false,
                                    "bFilter": false,
                                ...
                                $('#doubleinner3_' + jPos + '_' + mPos).on("click", "td.control", function ()
                                {
                                    var nTr3 = this.parentNode;
                                    var k = $.inArray(nTr3, anOpen3);
                                    if (k === -1)
                                    {
                                        var kPos = oInnerTable2.fnGetPosition(nTr3);
                                        $('i', this).attr('class', "fa fa-minus");
                                        ...
                                        $.ajax({
                                            ...
                                            success: function (result) {
                                                if (result !== null) {
                                                    benListJson = result;
                                                    console.log(result);
                                                    get_json_ben('beneficiaryTab', gpCode);
                                                    var ltable = $('#beneficiaryTab').html();
                                                    //console.log(ltable);
                                                    var nDetailsRow1 = oInnerTable2.fnOpen(nTr3, fnFormatDetails1(ltable, mPos, kPos, 4), 'details');
                                                    $('div.innerDetails', nDetailsRow1).slideDown();
                                                    anOpen3.push(nTr3);
                                                    var oInnerTable3 = $('#doubleinner4_' + mPos + '_' + kPos).dataTable({
                                                        "bRetrieve": true,
                                                        "bJQueryUI": true,
                                                        "bSort": false,
                                                        "iDisplayLength": 50,
