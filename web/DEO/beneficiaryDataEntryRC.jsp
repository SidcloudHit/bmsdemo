<%-- 
    Document   : beneficiaryDataEntryRC
...
<%
    ...
    if (request.getParameter("rcm") != null) {
        rcm = Security.SecurityClass.killchar(request.getParameter("rcm"));
        msg = Common.RCDataCheck.verifyRCByMemID(rcm);
    ...
    <head>
        ...
        <script language="javascript" type="text/javascript">
            window.onload = function () {
                var edit = '<%= edit%>';
                if (parseInt(edit) === 1) {
                    $("#txtBeneficiaryName").val('<%= jsobj.getString("member_name_en")%>');
                    $("#txtFathersName").val('<%= jsobj.getString("father_name_en")%>');
                    $("#txtDateOfBirth").val('<%= jsobj.get("member_dob").toString()%>');
                    var gender = '<%= jsobj.get("gender").toString()%>';
                    if (gender === 'M') {
                    ...
                    $("#txtRationCardNo").val('<%= jsobj.get("ration_card_no").toString()%>');
                    $("#txtRationCardMem").val('<%= jsobj.get("member_id").toString()%>');
                    $("#txtUIDCardNo").val('<%= jsobj.get("uid").toString()%>');
                    $('#ddlDistrict1').val('<%= jsobj.get("district_code").toString()%>');
