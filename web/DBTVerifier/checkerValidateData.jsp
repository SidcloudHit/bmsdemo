...
<html>
    <head>
        ...
        <title>BMS || Checker Validate  Data </title>
        ...
        $(document).ready(function () {
            ...
            $('#btnSearch').on('click', function (e) {
                ...
                var table = $('#contenttable').DataTable({
                    ...
                    "scrollX": true,
                    "ajax": {
                        "url": "${pageContext.request.contextPath}/AjaxMethod?dbtcheckermatch=1&schemeid=" + schemeid + "&match_strength=" + strength,
                        "type": "POST",
                        ...
                        },
                        {
                            render: function (data, type, full, meta) {
                                return '<input type="hidden"  id="benef_name_matchresult_dist_leven' + meta.row + '" value="' + full['benef_name_matchresult_dist_leven'] + '" />\n\
                                                                 ...
                                                                 <button type="button" class="btn btn-primary mb-2 matchdata" value="V~' + full['sl_no'] + '~' + full['scheme_code'] + '~' + full['postrcvalidation_rc_no'] + '~' + full['tempid'] + '~' + schemeid + '">Verify</button>\n\
                                                                 <button type="button" class="btn btn-danger mb-2 disapprove" value="D~' + full['sl_no'] + '~' + full['scheme_code'] + '~' + full['postrcvalidation_rc_no'] + '~' + full['tempid'] + '~' + schemeid + '">Disapprove</button>';
                            },
                            targets: 4,
                        },
                    ],
                ...
                $('#contenttable tbody').on('click', 'button.disapprove', function () {
                    ...
                    var benef_name = $("#benef_name" + row_id).val();
                    var benef_rc_mem = $("#benef_rc_mem" + row_id).val();
                    var r = confirm("Are you sure to disapproved this benefciary?\n\nName : " + benef_name + "\nRC Member Id : " + benef_rc_mem);
                    if (r == true) {
                        $.ajax({
                            url: "${pageContext.request.contextPath}/saveMatchdata?match_val=" + match_val,
                            type: "POST",
                            ...
                            success: function (data) {
                                if (data == "ok") {
                                    ...
                                    // alert("Data Updated Success Fully");
                                    $("#msgSucccess").css("display", "block");
                                    $("#msgsuccess").text("Data Updated as Disapproved Successfully.\nName: " + benef_name + ". \nRC Memberid: " + benef_rc_mem);
                                    $("#msgFail").css("display", "none");
                                    $("#msgfail").text("");
                                    $("html, body").animate({scrollTop: $("#returnMsg").offset().top}, "slow");
                                } else {
                                    ...
                                    $("#msgsuccess").text("");
                                    $("#msgFail").css("display", "block");
                                    $("#msgfail").text("No Data Updated as Disapproved. Try again");
                                    $("html, body").animate({scrollTop: $("#returnMsg").offset().top}, "slow");
                                }
                            }
